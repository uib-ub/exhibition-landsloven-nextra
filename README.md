...
The images are processed in two steps:
1. `copy-and-rename-images.js` copies the images to the `card-images` folder and renames them based on the filename and folder structure.
2. `process-images.js` processes the images to ensure they match the target aspect ratio of 4:3.

```bash
# Run the script
node scripts/copy-and-rename-images.js
